...
func TestGetSvcNamespace(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, got1, err := GetSvcNamespace(tt.args.address)
			if (err != nil) != tt.wantErr {
				t.Errorf("GetSvcNamespace() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.wantSvcName {
				t.Errorf("GetSvcNamespace() got = %v, want %v", got, tt.wantSvcName)
			}
			if got1 != tt.wantNamespace {
				t.Errorf("GetSvcNamespace() got1 = %v, want %v", got1, tt.wantNamespace)
			}
		})
	}
}
